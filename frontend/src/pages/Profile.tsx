...
const Profile = () => {
  ...
  const fetchInvites = async () => {
    try {
      const { data }: { data: UserInviteResponse } = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/users/invites`,
        { withCredentials: true }
      );
  ...
  const handleAvatarUpload = async (file: File) => {
    ...
    try {
      const { data }: { data: UserResponse } = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/users/upload/avatar`,
        formData,
        {
  ...
  const handleCheckoutInvitation = async (
    choice: string,
    userId: string,
    eventId: string
  ) => {
    try {
      await axios.put(
        `${import.meta.env.VITE_BASE_URL}/events/invite/${userId}/${eventId}`,
        { choice },
        { withCredentials: true }
      );
      // setEvents(data.user.events);
  ...
  const handleDeleteMember = async (eventId: string, memberId: string) => {
    try {
      axios.put(
        `${import.meta.env.VITE_BASE_URL}/events/member/del/${eventId}`,
        { memberId },
        { withCredentials: true }
      );
      // setEvents(data.user.events);
  ...
  if (userContext?.user && !userContext?.user?.isVerified) {
    return (
      <div className="text-center mt-8">
        You are not verified. Redirecting to verify...
      </div>
    );
  }
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-b from-[#1f021c] via-[#190341] to-[#22071b] text-white">
      <div className="bg-gray-900 rounded-lg shadow-lg p-8 my-6 border border-gray-400">
        ...
        {id &&
          userContext?.user &&
          ["ADMIN", "MODERATOR"].includes(userContext?.user?.role) && (
            <div>{id}</div>
          )}
        ...
        <div className="flex flex-col lg:flex-row items-center justify-center md:space-x-8 gap-5">
          <div className="flex flex-col md:flex-row items-center md:space-x-8 gap-5">
            <div className="relative h-32 w-32 rounded-full border-2 flex justify-center items-center">
              ...
              userContext?.user?.avatar?.length > 0 ? (
                <img
              ...
              <div
                ...
                className="absolute border border-slate-300 bottom-0 right-0 bg-slate-200 text-black rounded-full p-2"
              >
                <FaCamera size={20} />
              </div>
              <input
            ...
            {open && (
              <div className="text-black fixed inset-0 bg-opacity-30 backdrop-blur flex justify-center items-center z-20">
                <div className="bg-white p-8 rounded-lg shadow-lg w-full md:w-[60%] lg:w-[40%]">
                  ...
                  <form
                    className="mt-8 max-h-[70vh] overflow-y-scroll hide-scrollbar"
                    ...
                    {invites ? (
                      <>
                        {invites?.map((invite, index) => (
                          <div
                            key={index}
                            className="flex flex-col md:flex-row justify-between items-center bg-slate-200 p-2 rounded-lg"
                          >
                            <div>
                              <Link
                                className="underline"
                                to={`/event?id=${invite?.eventId._id}`}
                              >
                                Event: {invite.eventId.title}
                              </Link>
                    ...
                    <div className="mt-8 flex flex-col justify-center space-y-2">
                      <div>
                        <p className="text-xl font-semibold">
                          Event Title: {currentEvent?.eventId.title}
                        </p>
                        <p>
                          Event Date:{" "}
                          {new Date(
                            currentEvent?.eventId.eventDate!
                          ).toLocaleDateString("en-GB")}
                        </p>
                        <p>
                          Event Time:{" "}
                          {moment
                            .utc(currentEvent?.eventId.eventDate)
                            .format("hh:mm A")}{" "}
                          onwards
                        </p>
                        <p>isGroup: {currentEvent.isGroup.toString()}</p>
                        <p>Group Leader: {currentEvent.group?.leader?.email}</p>
                        {currentEvent.group?.members &&
                          currentEvent.group?.members?.length > 0 && (
                            <>
                              <p>Group Members: </p>
                              <div>
                                {currentEvent.group?.members?.map(
                                  (member, index) => (
                                    <div key={index} className="pl-6">
                                      {member.user.email} - {member?.status}
                                      {userContext?.user?._id ===
                                        currentEvent.group?.leader?._id && (
                                        <button
                                          className="bg-slate-200 text-red-500 m-1 p-1 rounded-lg"
                                          onClick={() =>
                                            handleDeleteMember(
                                              currentEvent.eventId._id,
                                              member.user._id
                                            )
                                          }
                                        >
                        ...
                        {currentEvent.paymentRequired && (
                          <>
                            <p>
                              Payment Status: {currentEvent?.payment?.status}
                            </p>
                            <p>
                              TransactionId:{" "}
                              {currentEvent?.payment?.transactionId}
                            </p>
                            {currentEvent?.payment?.paymentImage && (
                              <Link
                                onClick={(e) => e.stopPropagation()}
                                to={currentEvent?.payment?.paymentImage}
                                target="blank"
                              ...
                              >
                                ...
                                <img
                                  ...
                                  src={currentEvent?.payment?.paymentImage}
                                  alt={currentEvent.payment.transactionId}
                                />
                              </Link>
                            )}
                            <p>
                              Physical Verified:{" "}
                              {currentEvent?.physicalVerification?.status
                                ? "TRUE"
                                : "FALSE"}
                            </p>
                          </>
                        ...
                        <Link
                          to={`/event?id=${currentEvent?.eventId._id}`}
                          className="underline"
                        >
                          Go to events page
                        </Link>
                        ...
                        <div className="mt-2 text-sm">
                          ...
                          {event?.payment?.paymentImage && (
                            ...
                            >
                              ...
                              <img
                                className="h-10 w-10 rounded-lg"
                                src={event.payment.paymentImage}
                                alt={event.payment.transactionId}
                              />
                            </Link>
                          )}
                        </div>
